...
def load_data():
    ...
    entities = ['CACEIS SA', 'CACEIS Bank', 'Fund Administration', 'CACEIS (Corporate)']
    data = []
    for year in years:
        for entity in entities:
            hcva = np.random.randint(125, 165) + (year - 2023) * 7
            kti = np.random.uniform(0.68, 0.88) + (year - 2023) * 0.02
            skill_decay = np.random.uniform(0.08, 0.22) - (year - 2023) * 0.02
            re_score = np.random.uniform(3.4, 4.8)
            spe = np.random.uniform(0.18, 0.38) + (year - 2023) * 0.04
            chhi = ((hcva/200) * 0.3) + (kti * 0.2) + ((1-skill_decay) * 0.2) + (re_score/5 * 0.15) + (spe * 0.15)
            data.append({
                "Year": year, "Legal Entity": entity, "HCVA (k€)": round(hcva, 2),
                "KTI (%)": round(kti * 100, 2), "Skill Decay (%)": round(skill_decay * 100, 2),
                "RE-Score": round(re_score, 2), "SPE (%)": round(spe * 100, 2), "CHHI Index": round(chhi * 100, 2)
            })
...
if app_mode == "Live Dashboard":
    ...
    options_entity = ["Select All"] + list(df['Legal Entity'].unique())
    selected_entities = st.sidebar.multiselect("Legal entity Scope", options=options_entity, default=["Select All"])


    def get_filtered_df(year, entities_list):
        if "Select All" in entities_list or not entities_list:
            return df[df['Year'] == year]
        return df[(df['Year'] == year) & (df['Legal Entity'].isin(entities_list))]


    df_current = get_filtered_df(selected_year, selected_entities)
    has_previous = selected_year > min(df['Year'])
    if has_previous:
        df_prev = get_filtered_df(selected_year - 1, selected_entities)
    ...
    for i, tab in enumerate(tabs):
        ...
        with tab:
            ...
            with col_graph:
                entity_list = list(df['Legal Entity'].unique()) if "Select All" in selected_entities else selected_entities
                df_line = df[df['Legal Entity'].isin(entity_list)].copy()
                df_overall = df.groupby('Year')[conf['col']].mean().reset_index()
                df_overall['Legal Entity'] = 'CACEIS Average'
                df_plot = pd.concat([df_line, df_overall], ignore_index=True)
                fig = px.line(df_plot, x="Year", y=conf['col'], color="Legal Entity", markers=True, color_discrete_sequence=[CACEIS_STEEL_BLUE, CACEIS_RED, CACEIS_GREY, CACEIS_GREY_BLUE])
                fig.update_xaxes(dtick=1)
                fig.update_traces(patch={"line": {"color": "black", "width": 3, "dash": "dash"}}, selector={"name": "CACEIS Average"})
